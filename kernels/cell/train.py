...
TRANSFORMER_IMAGE_SIZE = 56 # 56 pixel is good for ~0.125nm per pixel (~40x)
TRANSFORMER_PATCH_SIZE = 7 # 56 pixel is good for ~0.125nm per pixel (~40x)
...
def train(opt):
    ...
    if opt.tensorboard:
        ...
        tb.configure(argv=[None, '--logdir', os.path.join(opt.output_folder, opt.model_prefix, "runs"), '--port', opt.tensorboard_port])
        url = tb.launch()
        print(f"Tensorboard listening on {url}")
        webbrowser.open(url)
        tensorboard_writer = SummaryWriter(os.path.join(opt.output_folder, opt.model_prefix, "runs"))
    ...
    if opt.model == 'ae':
        ...
        generator = AE(geneNum=gene_num, latentDim=opt.latent_dim)
        
    elif opt.model == 'vae':
        from models.vae import VAE
        generator = VAE(geneNum=gene_num, latentDim=opt.latent_dim)
        
    elif opt.model == 'vqvae':
        from models.vqvae import VQVAE
        generator = VQVAE(gene_num=gene_num, latent_dim=opt.latent_dim, num_embeddings = 1024, commitment_cost = 0.25, decay = 0.99)
        
    elif opt.model == 'vit':
        from models.vit import ViT
        generator = ViT(image_size = TRANSFORMER_IMAGE_SIZE,
                        patch_size=TRANSFORMER_PATCH_SIZE,
                        num_classes = gene_num,
                        dim = 512,
                        depth = 6,
                        heads = 8,
                        mlp_dim = 512,
    ...
    elif opt.model == "simplevit":
        from models.simplevit import SimpleViT
        generator = SimpleViT(image_size = TRANSFORMER_IMAGE_SIZE,
                        patch_size=TRANSFORMER_PATCH_SIZE,
                              num_classes = gene_num,
                              dim = 512,
                              depth = 6,
        ...
        generator = CaiT(image_size = TRANSFORMER_IMAGE_SIZE,
                        patch_size=TRANSFORMER_PATCH_SIZE,
                        num_classes = gene_num,
                        dim = 512,
    ...
    #                      layer_dropout = 0.05)
        ...
        generator = ResNet18(num_classes=gene_num)
        
    elif opt.model == 'resnet34':
        from models.resnet import ResNet34
        generator = ResNet34(num_classes=gene_num)
        
    elif opt.model == 'resnet50':
        from models.resnet import ResNet50
        generator = ResNet50(num_classes=gene_num)
        
    elif opt.model == 'resnet101':
        from models.resnet import ResNet101
        generator = ResNet101(num_classes=gene_num)
        
    elif opt.model == 'resnet152':
        from models.resnet import ResNet152
        generator = ResNet152(num_classes=gene_num)
        
    elif opt.model == 'vgg11':
        from models.vgg import VGG
        generator = VGG('VGG11', num_classes=gene_num)
        
    elif opt.model == 'vgg13':
        from models.vgg import VGG
        generator = VGG('VGG13', num_classes=gene_num)
        
    elif opt.model == 'vgg16':
        from models.vgg import VGG
        generator = VGG('VGG16', num_classes=gene_num)
        
    elif opt.model == 'vgg19':
        from models.vgg import VGG
        generator = VGG('VGG19', num_classes=gene_num)
                
    elif opt.model == 'convmixer':
        from models.convmixer import ConvMixer
        generator = ConvMixer(256, 
                              16, 
                              kernel_size=8, 
                              patch_size=1, 
                              n_classes=gene_num)
        ...
        generator = MLPMixer(image_size = TRANSFORMER_IMAGE_SIZE,
                             patch_size=TRANSFORMER_PATCH_SIZE,
                             channels = 3,
                             dim = 512,
                             depth = 6,
        ...
        generator = swin_t(window_size=7,
                           num_classes=gene_num,
                           downscaling_factors=(2,2,2,1))

    elif opt.model == 'swin_s':   
        from models.swin import swin_s
        generator = swin_s(window_size=7,
                           num_classes=gene_num,
                           downscaling_factors=(2,2,2,1))
        
    elif opt.model == 'swin_b':   
        from models.swin import swin_b
        generator = swin_b(window_size=7,
                           num_classes=gene_num,
                           downscaling_factors=(2,2,2,1))
        
    elif opt.model == 'swin_l':   
    ...
    discriminator = Discriminator(gene_num=gene_num, subtype_num=subtype_num)
    ...
    scheduler_G = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_G, opt.n_epochs)
    scheduler_D = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_D, opt.n_epochs)
    ...
    for epoch in optimizationRange:
        ...
        for i, (imgs, uuid, trns, sbtps) in enumerate(trainingDataLoader):
            ...
            elif opt.model == 'vqvae': vq_loss = generator.vq_loss
            ...
            elif opt.model == 'vqvae': vq_loss_weighted = opt.vq_loss_weight*vq_loss
            ...
            elif opt.model == 'vqvae': g_loss_weighted = (adv_loss_weighted + recon_loss_weighted + vq_loss_weighted + cell_corr_loss_weighted + gene_corr_loss_weighted) / 5
            ...
            if opt.tensorboard:
                ...
                elif opt.model == 'vqvae' or opt.model == 'vae': tensorboard_writer.add_scalar("training_loss/vq_loss", vq_loss.item(), epoch)
        ...
        elif opt.model == 'vqvae': val_vq_loss = 0
        ...
        for i, (imgs, uuid, trns, sbtps) in enumerate(validationDataLoader):
            ...
            if opt.tensorboard:
                ...
                elif opt.model == 'vqvae': tensorboard_writer.add_scalar("validation_loss/vq_loss", vq_loss.item(), epoch)
            ...
            elif opt.model == 'vqvae': val_vq_loss += vq_loss.item()
        ...
        elif opt.model == 'vqvae': val_vq_loss /= val_count
        ...
        if opt.checkpoint_pass_rate > 0.0:
            if val_gen_cls_acc >= last_gen_cls_acc*opt.checkpoint_pass:
                ...
                torch.save({
                    ...
                    }, os.path.join(opt.output_folder, opt.model_prefix, "checkpoint.pt"))
            else:
        
                checkpoint = torch.load(os.path.join(opt.output_folder, opt.model_prefix, "checkpoint.pt"))
        ...
        if opt.test_interval != 0 and (epoch+1) % opt.test_interval == 0:
            ...
            result_adata.write_h5ad(os.path.join(opt.output_folder, opt.model_prefix, validationH5adFilename), compression='gzip')
        ...
        if opt.saving_interval != 0 and (epoch+1) % opt.saving_interval == 0:
            ...
            torch.save({'generator_model_state_dict': generator.state_dict(),
                        ...
                        'discriminator_optimizer_state_dict': optimizer_D.state_dict(),
                        'parameters': param_json_data
                        }, os.path.join(opt.output_folder, opt.model_prefix, modelFilename))
               
    scheduler_G.step(epoch-1) # step cosine scheduling
    scheduler_D.step(epoch-1) # step cosine scheduling
    ...
    if opt.generate_report:
        with open(os.path.join(opt.output_folder, opt.model_prefix, "description.txt"), 'w') as f:
            f.write("parameters: {}\n".format(vars(opt)))
