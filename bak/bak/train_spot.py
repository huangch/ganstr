...
from utils import SpotCellImageDataset, weights_init_normal
...
parser.add_argument("-ld", "--latent_dim", type=int, default=32, help="dimensionality of the latent space")
parser.add_argument("-is", "--image_size", type=int, default=48, help="input image size")
parser.add_argument("-l1", "--l1_loss", action='store_true', help="using l1 loss")
parser.add_argument('-md','--model', type=str, default="ae", help="model, ae/vae/vqvae")
parser.add_argument("-dw", "--discrimination_weight", type=float, default=1, help="discriminative weight")
parser.add_argument("-aw", "--adversarial_weight", type=float, default=1, help="adversarial weight")
parser.add_argument("-rw", "--reconstruction_weight", type=float, default=1, help="reconstruction weight")
parser.add_argument("-xw", "--correlation_weight", type=float, default=1, help="correlation weight")
parser.add_argument("-xcw", "--cell_correlation_weight", type=float, default=1, help="correlation weight")
parser.add_argument("-xgw", "--gene_correlation_weight", type=float, default=1, help="correlation weight")
parser.add_argument("-cw", "--classification_weight", type=float, default=1, help="classification weight")
parser.add_argument("-klw", "--kl_divergence_weight", type=float, default=1, help="KL divergence weight")
parser.add_argument("-vqw", "--vq_loss_weight", type=float, default=1, help="VQ loss weight")
parser.add_argument("-mp", "--model_prefix", type=str, default="", help="model file path prefix")
parser.add_argument("-cp", "--checkpoint_pass", type=float, default=0.0, help="checkpoint pass rate")
parser.add_argument("-si", "--saving_interval", type=int, default=0, help="interval between model saving")
parser.add_argument("-ti", "--test_interval", type=int, default=0, help="interval between model testing")
...
parser.add_argument("-dp", "--detailed_progress", action='store_true', help="detailed progress")
...
for imageFileName in imageFileNameList:
    ...
    imageFileSpotId = os.path.normpath(imageFilePath).split(os.path.sep)[-1]
    
    assert existingImageFileExt == None or existingImageFileExt == imageFileExt
    existingImageFileExt = imageFileExt
    
    imageFileSpotIdList.append(imageFileSpotId)
    imageFilePathList.append(imageFilePath)
    imageFileUuidList.append(imageFileUuid)
    
imageData = pd.DataFrame({'imageFilePath': imageFilePathList, 'imageFileUuid':imageFileUuidList, 'imageFileSpotId':imageFileSpotIdList})
...
# Create datasets
trainingTransformedDataset = ImageDataset(trainingImageData, trainingTrnsData, trainingSubtypeData, imageFileExt=existingImageFileExt, imageSize=opt.image_size, imageMean=imageMean, imageStd=imageStd, trnsCountPerCell=trnsCountPerCell)
validationTransformedDataset = ImageDataset(validationImageData, validationTrnsData, validationSubtypeData, imageFileExt=existingImageFileExt, imageSize=opt.image_size, imageMean=imageMean, imageStd=imageStd, trnsCountPerCell=trnsCountPerCell)
...
if opt.final_test:
    ...
    # Create datasets
    totalTransformedDataset = ImageDataset(totalImageData, totalTrnsData, totalSubtypeData, imageFileExt=existingImageFileExt, imageSize=opt.image_size, imageMean=imageMean, imageStd=imageStd, trnsCountPerCell=trnsCountPerCell)
